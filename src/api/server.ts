...
export async function getChallenge(challengeId: number) {
	...
	return db
		...
		.from(Challenges)
		.leftJoin(UserChallenges, and(eq(Challenges.id, UserChallenges.challengeId), eq(UserChallenges.userId, user.id)))
...
export async function getUserChallengeScores() {
	...
	const scores = db
		.select({
			...
			challengeName: Challenges.name,
			challengeDescription: Challenges.description,
			userScore: UserChallenges.score,
		})
		.from(Challenges)
		.leftJoin(UserChallenges, and(eq(Challenges.id, UserChallenges.challengeId), eq(UserChallenges.userId, user.id)))
